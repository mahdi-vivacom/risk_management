...
class CommissionDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->addColumn('action', function ($query) {
                $name = 'Commission';
                if (auth()->user()->can('commissions.edit')) {
                    $a = '<a href="' . route('commissions.edit', ['commission' => $query->id]) . '" class="btn btn-outline-info btn-sm" title="Edit' . $name . '"><i class="ri-edit-box-line"></i></a>';
                }
                if (auth()->user()->can('commissions.destroy')) {
    ...
    {
        return $this->builder()
            ->setTableId('Commission-DataTable')
