                    ...
                    <form action="{{ route ( 'country-areas.update', [ 'country_area' => $countryArea->id ] ) }}"
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="skill[]" id="skill" multiple="multiple"
                                ...
                                @foreach ( $skills as $skill )
                                    <option value="{{ $skill->id }}" {{ $countryArea->Skill->contains ( 'id', $skill->id ) ? 'selected' : '' }}>
                                        {{ $skill->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="country_id" id="country_id"
                                ...
                                @foreach ( $countries as $country )
                                    <option value="{{ $country->id }}" {{ old ( 'country_id', $countryArea->country_id ) == $country->id ? 'selected' : '' }}>{{ $country->name }}</option>
                                @endforeach
                            </select>
                            @error( 'country_id' )
                                <div class="invalid-feedback">{{ $message }}</div>
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="timezone" id="timezone"
                                ...
                                @foreach ( $timezones as $time )
                                    <option value="{{ $time }}" {{ old ( 'timezone', $countryArea->timezone ) == $time ? 'selected' : '' }}>
                                        {{ $time }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="text-center">
                            ...
                            <button type="submit" class="btn btn-outline-success btn-sm"
                                onClick="this.form.submit(); this.disabled=true; this.innerText='Updating .....';">@lang(
                                'admin_fields.update' )</button>
                        </div>
                    </form>
                </div>
            </div>
...
<script>
    ...
    function changeBill(type) {
        $("#start_time").val('');
        $("#start_day").val('');
        $("#start_day").prop('selected', false);
        $("#start_date").val('');
        $("#start_date").prop('selected', false);
        //
        switch (type) {
            case "1":
