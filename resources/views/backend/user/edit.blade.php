            ...
            <div class="card">
                <div class="card-body pt-3">
                    <form class="row g-3" enctype="multipart/form-data"
                        action="{{ route ( $route . '.update', [ 'user' => $user->id ] ) }}" method="post" novalidate>
                        @csrf
                        @method( 'patch' )
                        <div class="col-md-6">
                            ...
                            <input id="name" type="text" name="name"
                                ...
                                value="{{ old ( 'name', $user->name ) }}" placeholder="@lang( 'admin_fields.name' )"
                                autocomplete="name">
                            @error( 'name' )
                        ...
                        <!-- <div class="col-md-6">
                            ...
                            <input id="email" type="email" name="email"
                                ...
                                value="{{ old ( 'email', $user->email ) }}" placeholder="@lang( 'admin_fields.email' )"
                                autocomplete="email">
                            @error( 'email' )
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="role" id="role"
                                ...
                                @foreach ( $roles as $role )
                                    <option value="{{ $role->name }}" {{ old ( 'role', $user->roles->pluck ( 'id' )->first () ) == $role->id ? 'selected' : '' }}>
                                        {{ $role->display_name }}
                                    </option>
                                @endforeach
                            </select>
