                ...
                <div class="card">
                    <div class="card-body pt-3">
                        <form class="row g-3" enctype="multipart/form-data"
                            action="{{ route($route . '.update', ['professional' => $professional->id]) }}" method="post"
                            novalidate>
                            @csrf
                            @method('patch')
                            <div class="col-md-6">
                                ...
                                <input id="first_name" type="text" name="first_name"
                                    ...
                                    placeholder="@lang('admin_fields.first_name')" autocomplete="first_name">
                                @error('first_name')
                            ...
                            <div class="col-md-6">
                                ...
                                <input id="last_name" type="text" name="last_name"
                                    ...
                                    value="{{ old('last_name', $professional->last_name) }}" placeholder="@lang('admin_fields.last_name')"
                                ...
                                <select name="country_id" id="country_id"
                                    ...
                                    @foreach ($countries as $country)
                                        <option value="{{ $country->id }}" alpha_2="{{ $country->iso_code_2 }}"
                                            {{ old('country_id', $professional->country_id) == $country->id ? 'selected' : '' }}
                                ...
                                <select name="country_area_id" id="country_area_id"
                                    ...
                                    {{-- @foreach ($areas as $area)
                                        <option value="{{ $area->id }}"
                                            {{ $professional->country_area_id == $area->id ? 'selected' : '' }}>
                                            {{ $area->name }}</option>
                                    @endforeach --}}
                                </select>
                            ...
                            <div class="col-md-6">
                                ...
                                <div class="input-group mb-3">
                                    <input id="phone" type="text" name="phone_number"
                                        ...
                                        value="{{ old('phone_number', $professional->phone_number) }}"
                                        placeholder="@lang('admin_fields.phone_number')" autocomplete="phone_number">
                                    @error('phone_number')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                            ...
                            <div class="col-md-6">
                                ...
                                <input id="email" type="email" name="email"
                                    ...
                                    value="{{ old('email', $professional->email) }}" placeholder="@lang('admin_fields.email')"
                                    autocomplete="email">
                                @error('email')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            ...
                            <div class="col-md-6">
                                ...
                                <select name="gender" id="gender"
                                    ...
                                    <option value="1" {{ $professional->gender == 1 ? 'selected' : '' }}>@lang (
                                        'admin_fields.male',
                                        )</option>
                                    <option value="2" {{ $professional->gender == 2 ? 'selected' : '' }}>@lang (
                                        'admin_fields.female',
                                        )</option>
                                    <option value="3" {{ $professional->gender == 3 ? 'selected' : '' }}>@lang (
                                        'admin_fields.others',
                                        )</option>
                                </select>
                                @error('gender')
                                ...
                                <select name="skill" id="skill"
                                    ...
                                    @foreach ($skills as $skill)
                                        <option value="{{ $skill->id }}"
                                            {{ old('skill', $professional->Skill['0']->id) == $skill->id ? 'selected' : '' }}>
                                            {{ $skill->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="text-center">
                                <button type="submit" class="btn btn-success btn-sm">@lang (
                                    'admin_fields.update',
                                    )</button>
...
@section('custom_js')
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            let countryId = '{{ $professional->country_id }}';
            ...
            function getCountryArea(countryId) {
                $.ajax({
                    ...
                    success: function(response) {
                        let areaId = '{{ $professional->country_area_id }}';
