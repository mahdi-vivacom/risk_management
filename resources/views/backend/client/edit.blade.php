                ...
                <div class="card">
                    <div class="card-body pt-3">
                        <form class="row g-3" enctype="multipart/form-data"
                            action="{{ route('clients.update', ['client' => $client->id]) }}" method="post" novalidate>
                            @csrf
                            @method('patch')
                            <div class="col-md-6">
                                ...
                                <input id="first_name" type="text" name="first_name"
                                    ...
                                    value="{{ old('first_name', $client->first_name) }}" placeholder="@lang('admin_fields.first_name')"
                            ...
                            <div class="col-md-6">
                                ...
                                <input id="last_name" type="text" name="last_name"
                                    ...
                                    value="{{ old('last_name', $client->last_name) }}" placeholder="@lang('admin_fields.last_name')"
                                ...
                                <select name="country_id" id="country_id"
                                    ...
                                    @foreach ($countries as $country)
                                        <option value="{{ $country->id }}"
                                            {{ $client->country_id == $country->id ? 'selected' : '' }}>{{ $country->name }}
                                        </option>
                                    @endforeach
                                </select>
                                @error('country_id')
                                ...
                                <select name="country_area_id" id="country_area_id"
                                    ...
                                    {{-- @foreach ($areas as $area)
                                        <option value="{{ $area->id }}"
                                            {{ $client->country_area_id == $area->id ? 'selected' : '' }}>
                                            {{ $area->name }}</option>
                                    @endforeach --}}
                                </select>
                            ...
                            <div class="col-md-6">
                                ...
                                <input id="email" type="email" name="email"
                                    ...
                                    value="{{ old('email', $client->email) }}" placeholder="@lang('admin_fields.email')"
                                    autocomplete="email">
                                @error('email')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            ...
                            <div class="col-md-6">
                                ...
                                <select name="gender" id="gender"
                                    ...
                                    <option value="1" {{ $client->gender == 1 ? 'selected' : '' }}>@lang (
                                        'admin_fields.male',
                                        )</option>
                                    <option value="2" {{ $client->gender == 2 ? 'selected' : '' }}>@lang (
                                        'admin_fields.female',
                                        )</option>
                                    <option value="3" {{ $client->gender == 3 ? 'selected' : '' }}>@lang (
                                        'admin_fields.others',
                                        )</option>
                                </select>
                                @error('gender')
                            ...
                            <div class="col-md-6">
                                ...
                                <select name="status" id="status"
                                    ...
                                    <option value="1" {{ $client->status == 1 ? 'selected' : '' }}>@lang (
                                        'admin_fields.active',
                                        )</option>
                                    <option value="0" {{ $client->status == 0 ? 'selected' : '' }}>@lang (
                                        'admin_fields.inactive',
                                        )</option>
                                </select>
                                @error('status')
                                    <div class="invalid-feedback">{{ $message }}</div>
            ...
            function getCountryArea(countryId) {
                $.ajax({
                    ...
                    success: function(response) {
                        let areaId = '{{ $client->country_area_id }}';
